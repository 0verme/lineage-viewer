...
This document freezes the `0.1.0` public browser API. The package is currently Alpha and the API may still change. After publication, install it with `npm install lineage-viewer@alpha`. Only the root and `./define` package exports are supported.
